...
def getNextIPV4Address(addr):
    """Get the next succeeding IP address."""
    ...
    mask = addr.split('/')[1]
    return ipv4_int_to_str(i) + "/" + mask


def prefixToWildcardMask(prefix):
    """Transform a prefix (as string) to a wildcard mask (as a string)."""
    mask = prefix_to_netmask(int(prefix))
    inverted_mask = ~mask & _ipv4_max
    return ipv4_int_to_str(inverted_mask)
...
class TestTheMethods(unittest.TestCase):
    ...
    def test_nextIPV4Address(self):
        self.assertEqual(getNextIPV4Address('192.168.128.2/24'), '192.168.128.3/24')
        self.assertEqual(getNextIPV4Address('192.168.128.254/24'), '192.168.129.1/24')

    def test_prefixToWildcardMask(self):
        self.assertEqual(prefixToWildcardMask('24'), '0.0.0.255')
        self.assertEqual(prefixToWildcardMask('16'), '0.0.255.255')
